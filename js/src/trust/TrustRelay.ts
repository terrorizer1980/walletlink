...
export class TrustRelay implements Relay {
  ...
  public attach(el: Element): void {
    if (this.attached) {
      throw new Error("TrustWeb3Provider is already attached")
    }
    (TrustWeb3Provider.prototype as any).isTrust = true;
    (TrustWeb3Provider.prototype as any).trustMessage = this.handleMessage;
    (TrustWeb3Provider.prototype as any).sendResponse = (id: string,addresses: string[]) => {
      this.handleMessage(
        {data: {id, response: { result: addresses }}} as MessageEvent
      )
    }
    (TrustWeb3Provider.prototype as any).sendError = (id: string, message: string) => {
      this.handleMessage(
        {data: {id, response: { errorMessage: message }}} as MessageEvent
      )
    }
    this.attached = true
  }
  ...
  private postIPCMessage(message: RelayMessage): void {
